...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { conversationId }: VideoGenerationRequest = await req.json()
    ...
    const veoResponse = await fetch(
      `https://us-central1-aiplatform.googleapis.com/v1/projects/${projectId}/locations/us-central1/publishers/google/models/veo-3.0-generate-preview:predictLongRunning`,
      {
        method: 'POST',
        headers: {
    ...
    return new Response(JSON.stringify({ success: true, operationId }), {
...
async function pollVideoGeneration(conversationId: string, operationName: string, initialAccessToken: string, projectId: string) {
  const supabase = createClient(
    Deno.env.get('SUPABASE_URL')!,
    Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!
  )
  ...
  while (pollCount < maxPolls) {
    ...
    try {
      ...
      const statusResponse = await fetch(
        `https://us-central1-aiplatform.googleapis.com/v1/projects/${projectId}/locations/us-central1/publishers/google/models/veo-3.0-generate-preview:fetchPredictOperation`,
        {
          method: 'POST',
          headers: {
...
async function downloadAndStoreVideo(gcsUri: string, conversationId: string, accessToken: string): Promise<string> {
  const supabase = createClient(
    Deno.env.get('SUPABASE_URL')!,
    Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!
  )
...
async function sendCompletionMessage(conversationId: string, videoUrl: string) {
  const supabase = createClient(
    Deno.env.get('SUPABASE_URL')!,
    Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!
  )
  ...
  console.log('Completion message sent for conversation:', conversationId)
...
async function sendErrorMessage(conversationId: string) {
  const supabase = createClient(
    Deno.env.get('SUPABASE_URL')!,
    Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!
  )
